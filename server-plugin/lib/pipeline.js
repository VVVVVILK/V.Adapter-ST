...
// 同时供 admin.js 的 /admin/test、/admin/translate 使用（对应 Go 版 qwen_client.go 的 targetFromSettings）。
...
export async function generateImage(target, prompt, neg, size, mode) {
    ...
    if (mode === 'chat_only') {
        return generateViaChat(target, prompt, neg);
    }
    if (mode === 'openai') {
...
export async function postJSON(target, path, body, timeoutMs) {
    ...
    if (!base) {
        throw new Error('上游 API 地址未配置：请在管理面板「设置中心」填写 qwen_url');
    }
    ...
    try {
        resp = await fetch(base + path, {
            ...
            headers: {
                ...
                'Authorization': 'Bearer ' + (String(target.key ?? '').trim() || 'EMPTY'), // 本地免鉴权服务用占位符
            },
...
// 除 5xx 外，429（限流/风控/WAF 拦截）同样说明该接口当前用不了：
// 上游被风控时每次出图都会先撞一次 429 再转聊天兜底，既慢又刷屏。
function imagesEndpointUnavailable(err) {
    ...
    return /WAF|captcha|风控|quota_limit|验证码|限制/i.test(body);
}
